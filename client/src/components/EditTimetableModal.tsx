...
const EditTimetableModal: React.FC<EditTimetableModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl border border-slate-100 flex flex-col">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50 rounded-t-3xl shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Customize your master blueprint for {template.day}s</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto bg-slate-50/50 flex-1">
          <div className="space-y-3">
            ...
            {tasks.map((task, idx) => (
              <div key={task.taskId || idx} className="flex flex-col sm:flex-row gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm items-start sm:items-center">
                ...
                <div className="flex-1 w-full">
                  <input
                    ...
                    className="w-full text-sm font-bold text-slate-700 px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  />
                </div>
                ...
                <div className="w-full sm:w-40 shrink-0">
                  <select
                    ...
                    className="w-full text-xs font-semibold px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white transition-all text-slate-600"
                  >
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                  <input
                    ...
                    className="w-28 text-xs font-mono font-semibold px-2 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                  />
                ...
                <button
                  ...
                  className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors ml-auto sm:ml-0"
                  title="Remove Task"
                >
                  <Trash2 size={18} />
                </button>
          ...
          <button
            ...
            className="mt-4 w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-sm font-bold text-slate-500 flex items-center justify-center gap-2 hover:bg-slate-100 hover:text-slate-700 transition-colors"
          >
